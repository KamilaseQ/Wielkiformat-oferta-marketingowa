...
export function PackageDetail({ pkg, open, onOpenChange }: PackageDetailProps) {
  ...
  return (
    ...
    >
      ...
      <Block icon={Sparkles} title="Jak to wspiera Wielki Format" accent={pkg.accent}>
        <ul className="space-y-2.5">
          {pkg.supports.map((item) => (
            <li
              key={item}
              className="rounded-lg bg-white/[0.03] px-4 py-2.5 text-sm leading-relaxed text-ink-muted"
            >
              {item}
            </li>
          ))}
      ...
      {pkg.ecosystem && (
        <Block icon={Network} title="Schemat strategii" accent={pkg.accent}>
