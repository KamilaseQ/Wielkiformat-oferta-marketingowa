...
import { Check, Crown, ArrowRight, Plus } from "lucide-react";
  ...
  return (
    <div
      className={cn(
        ...
        isGold
          ? "border-gold-500/50 bg-gradient-to-b from-gold-500/[0.08] to-transparent shadow-2xl shadow-gold-500/10"
          : pkg.tag
            ? "border-brand-500/40 bg-gradient-to-b from-brand-500/[0.06] to-transparent shadow-lg shadow-brand-500/5"
            : "border-border bg-gradient-to-b from-white/[0.035] to-transparent hover:border-white/20",
      )}
    >
      {pkg.featured && (
        <div className="absolute -top-3 left-7 inline-flex items-center gap-1.5 rounded-full bg-gold-400 px-3 py-1 text-xs font-bold uppercase tracking-wider text-night">
          <Crown className="h-3.5 w-3.5" />
          Pełny ekosystem
        </div>
      )}
      {!pkg.featured && pkg.tag && (
        <div className="absolute -top-3 left-7 inline-flex items-center rounded-full bg-brand-500 px-3 py-1 text-xs font-bold uppercase tracking-wider text-white">
          {pkg.tag}
        </div>
      )}
      ...
      <p className="mt-5 text-sm leading-relaxed text-ink-muted">{pkg.goal}</p>
      ...
      <div
        className={cn(
          "mt-6 rounded-xl p-4 leading-relaxed",
          isGold
            ? "border border-gold-500/30 bg-gold-500/[0.07] text-base font-semibold text-ink"
            : "bg-white/[0.03] p-3 text-sm italic text-ink-muted",
        )}
      >
        {pkg.effect}
      </div>
